...
function tutorSystem(){
  ...
  const ch=(state.chapter&&state.chapter!=='all') ? ' currently studying the chapter "'+state.chapter+'"' : '';
  return [
    'You are Vault AI, a warm, patient teaching tutor for CBSE Class 11/12 and JEE Main & Advanced (India). Today you are tutoring '+subj+' for Class '+cls+ch+'.',
    'TEACHING METHOD — teach the thinking, not just the formula. For each topic:',
    '1) INTUITION first: open with a short everyday analogy that makes the idea click before any mathematics.',
    '2) Then give the formula/law and explain WHY each symbol is there and what the equation is really saying.',
    '3) EXAM PATTERN: note how JEE Main usually asks it vs where Advanced goes deeper or sets trap options; give a short step-by-step worked numerical.',
    '4) MISCONCEPTIONS: name the classic mistake students make and the warning sign that they are about to fall into it.',
    ...
    'Formatting: math in LaTeX with \\( ... \\) inline and \\[ ... \\] display; bold key terms with **term**. Use short paragraphs and bullets, not walls of text.',
...
function tutorHTML(){
  ...
  return '<div class="fv-drawer-head">'+
    ...
    '<div class="fv-chat-input"><textarea id="fvChatText" rows="1" placeholder="Ask anything — explain Bohr model, trick for SHM…"></textarea><button class="fv-send" id="fvSend">➤</button></div>';
}
...
function openTutor(){
  ...
  function renderSug(){
    ...
    const sug = ch ? ['Explain '+ch+' simply','Top JEE tricks for '+ch,'Common mistakes in '+ch,'3 practice problems on '+ch]
                   : ['Revision plan for '+SUBJ[state.subject],'Most important '+SUBJ[state.subject]+' chapters for JEE','Make a 1-page formula sheet'];
...
function openExplain(c){
  ...
  const prompt='Explain this JEE '+SUBJ[c.subject]+' formula for a Class '+c.cls+' student.\nTitle: '+c.t+'\nFormula: '+FV.strip(c.eq)+
